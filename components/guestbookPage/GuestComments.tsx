...
export default function GuestComments() {
    ...
    if (error)
        return (
            <pre className="text-xl text-gray-500">{JSON.stringify(error)}</pre>
        );
    ...
    return (
        <InfiniteScroll
            ...
            next={() => {
                return fetchMore({
                    variables: {
                        after: data.guestBooksConnection.pageInfo.endCursor,
                    },
                });
            }}
            ...
            <motion.ul className="relative mt-12 customLine before:top-0 borderLeft">
                {data.guestBooksConnection.edges.map((d) => (
                    ...
                    >
                        ...
                        <p className="mt-4 text-xl font-normal leading-10 tracking-wider text-gray-400">
                            {d.node.comment}
                        </p>
                    </motion.li>
                ))}
